...
create_tensor_map(bf16 *ptr, int global_height, int global_width) {
  CUtensorMap tma_map;
  ...
  CUresult res = cuTensorMapEncodeTiled(
      &tma_map, CU_TENSOR_MAP_DATA_TYPE_BFLOAT16, 3, gmem_addr, gmem_prob_shape,
...
template <uint32_t N, uint32_t K, uint32_t BLOCK_M, uint32_t BLOCK_N,
          uint32_t BLOCK_K, uint32_t SUPER_N, uint32_t NUM_M_BLOCKS,
          uint32_t CLUSTER_M, uint32_t CLUSTER_N, uint32_t NUM_STAGES>
struct GeMMBF16NN {
  static void Run(bf16 *out, bf16 *a, bf16 *b, uint32_t m, cudaStream_t stream,
                  int NUM_SMS) {
    static_assert(NUM_SMS % (CLUSTER_M * CLUSTER_N) == 0,
                  "num_sms must be divisible by CLUSTER_M * CLUSTER_N");
