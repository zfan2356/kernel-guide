...
/*
    ...
    - Move<nv_bfloat16, CopyAtom::OpS2R>
    - Move<float, CopyAtom::OpS2R>
    - Move<nv_bfloat16, CopyAtom::OpR2G>
...
template <> struct Move<nv_bfloat16, CopyAtom::OpS2R> {
    // use ld.shared
...
template <typename T>
    ...
    std::is_same_v<T, nv_bfloat162> struct Move<T, CopyAtom::OpS2R> {
    // use ld.shared
    template <uint32_t N = 1> __device__ __forceinline__ static void move(void* dst, void* src) {
        uint32_t src_ptr = static_cast<uint32_t>(__cvta_generic_to_shared(src));
        char* dst_ptr = static_cast<char*>(dst);
...
template <typename T>
    requires std::is_same_v<T, float> ||
    std::is_same_v<T, nv_bfloat162> struct Move<T, CopyAtom::OpG2R> {
    // use ld.shared
    template <uint32_t N = 1>
    __device__ __forceinline__ static void move(void* dst, const void* src) {
        if constexpr (N == 1) {
            *(reinterpret_cast<float*>(dst)) = __ldg(reinterpret_cast<const float*>(src));
        } else if constexpr (N == 2) {
            *(reinterpret_cast<float2*>(dst)) = __ldg(reinterpret_cast<const float2*>(src));
        } else if constexpr (N == 4) {
...
template <typename T>
    requires std::is_same_v<T, float> ||
    std::is_same_v<T, nv_bfloat162> struct Move<T, CopyAtom::OpR2G> {
    // use st.global.cs
    template <uint32_t N = 1> __device__ __forceinline__ static void move(void* dst, void* src) {
...
template <typename T>
    requires std::is_same_v<T, float> ||
    std::is_same_v<T, nv_bfloat162> struct Move<T, CopyAtom::OpR2S> {
    // use st.shared
    template <uint32_t N = 1> __device__ __forceinline__ static void move(void* dst, void* src) {
        char* src_ptr = static_cast<char*>(src);
...
template <typename T> requires std::is_same_v<T, nv_bfloat16> struct Move<T, CopyAtom::OpR2G> {
    template <uint32_t N = 1> __device__ __forceinline__ static void move(void* dst, void* src) {
