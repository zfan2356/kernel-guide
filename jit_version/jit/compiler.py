...
def get_nvcc_compiler() -> Tuple[str, str]:
    """
    Get the NVCC compiler path then check its version >= 12.3.
    """
    paths = []
...
def get_default_user_dir():
    return os.path.expanduser("~") + "/.kernels"
...
def build(name: str, arg_defs: tuple, code: str) -> Runtime:
    # Compiler flags
    cpp_standard = int(os.getenv("NVCC_OVERRIDE_CPP_STANDARD", 20))
    nvcc_flags = [
        ...
        + (",--verbose" if "PTXAS_VERBOSE" in os.environ else ""),
    ...
    enable_sass_opt = (
        ...
        and int(os.getenv("DISABLE_FFMA_INTERLEAVE", 0)) == 0
    )
    ...
    if os.getenv("JIT_DEBUG", None) or os.getenv("JIT_PRINT_NVCC_COMMAND", False):
